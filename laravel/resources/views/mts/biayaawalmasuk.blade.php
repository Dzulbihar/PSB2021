        ...
        <div class="card">
            <div class="card-body">  
                <div class="row">
                    ...
                    <div class="col-md-2">
                        <h2></h2><br><br>
                        <img src="{{ url('logo/mts.png')}}" width="100%">
                    </div>
                    <!-- --> 
                    <!-- kolom 2 --> 
                    <div class="col-md-10">
                        <h2></h2>
                        <h3> <b> Registrasi Jenjang MTs Askhabul Kahfi, {{ ($mts->fasilitas) }} </b> </h3>
...
<br><br>
      ...
      <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title"> <b> Data History Pembayaran {{ ($mts->namalengkap) }} </b> </h3>
        </div>
        <div class="panel-body">
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach($mts->registrasi as $registrasi)
                    <tr>
                        ...
                        <td><a href="{{ url('mts') }}/{{$mts->id}}/{{$registrasi->id}}/{{'deleteregistrasi'}}" class="btn btn-danger btn-sm" onclick="return confirm('Yakin mau dihapus')">Delete</a></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <a href="{{ url('mts') }}/{{$mts->id}}/{{('printregistrasimts')}}" target="_blank" class="btn btn-success float-right"></i> Print Registrasi </a>
        </div>
    </div>
...
<div class="modal fade" id="registrasi" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h3 class="modal-title" id="exampleModalLabel">Tambah Data Biaya Awal Masuk</h3>
  ...
  <div class="modal-body">
    ...
    <form action="{{url('mts')}}/{{$mts->id}}/addbiaya"  method="POST" enctype="multipart/form-data">
        {{ csrf_field() }}
        <!-- registrasi -->
        ...
        <div class="form-group">
            <div class="col-md-6">
                ...
                <input readonly name="jalur" type="text" class="form-control @error('jalur') is-invalid @enderror" id="jalur" aria-describedby="emailHelp" value="{{$mts->jalur}}" required autocomplete="jalur" autofocus>
            </div>

        <!-- Jenjang -->
            <div class="col-md-6">
                <label for="jenjang"> Jenjang </label>
                <input readonly name="jenjang" type="text" class="form-control @error('jenjang') is-invalid @enderror" id="jenjang" aria-describedby="emailHelp" value="{{$mts->status}}" required autocomplete="jenjang" autofocus>
            </div>
        </div>
        ...
        <div class="form-group">
            <div class="col-md-4">
                ...
                <input readonly name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" aria-describedby="emailHelp" value="{{$mts->namalengkap}}" required autocomplete="nama" autofocus>
            </div>

        <!-- fasilitas -->
            <div class="col-md-4">
                <label for="fasilitas"> Fasilitas </label>
                <input readonly name="fasilitas" type="text" class="form-control @error('fasilitas') is-invalid @enderror" id="fasilitas" aria-describedby="emailHelp" value="{{$mts->fasilitas}}" required autocomplete="fasilitas" autofocus>
            </div>

        <!-- jenis_kelamin -->
            <div class="col-md-4">
                <label for="jenis_kelamin"> Jenis Kelamin </label>
                <input readonly name="jenis_kelamin" type="text" class="form-control @error('jenis_kelamin') is-invalid @enderror" id="jenis_kelamin" aria-describedby="emailHelp" value="{{$mts->jenkel}}" required autocomplete="jenis_kelamin" autofocus>
            </div>
            <br><br><br>
        </div>
        <br><br><br>
